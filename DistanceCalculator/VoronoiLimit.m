...
% 31 October 2016: Bugfixes + addition of a routine which checks for a more recent version of the function on the Mathworks fileexchange
% --------------------------------------------------------------
...
for ij=1:length(C)
    if any(C{ij}==1)
        ...
        if length(ixa)==2 || length(ixb)==2
            ...
            centroidIN=false(length(ixa),length(ixb));
            sectionCROSS=false(length(ixa),length(ixb));
            for ic1=1:length(ixa)
                for ic2=1:length(ixb)
                    centroidIN(ic1,ic2)=inpolygon(x(ij),y(ij),[V(lV0+ixa(ic1),1);polygon(:,1);V(lV0+ixb(ic2),1)],[V(lV0+ixa(ic1),2);polygon(:,2);V(lV0+ixb(ic2),2)]);
                    [xi,~] = polyxpoly(polygon(:,1),polygon(:,2),[V(lV0+ixa(ic1),1);V(lV0+ixb(ic2),1)],[V(lV0+ixa(ic1),2);V(lV0+ixb(ic2),2)]);
                    sectionCROSS(ic1,ic2)=isempty(xi);
                end
            end
            selection=centroidIN.*sectionCROSS;
